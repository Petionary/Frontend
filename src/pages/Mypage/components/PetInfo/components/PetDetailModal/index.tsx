...
const PetDetailModal = ({ handleDetailToggle }: PetDetailModalProps) => {
  const initialPetInput = {
    id: mockPet?.id,
    name: mockPet?.name || '',
    species: mockPet?.species || '',
    detail_species: mockPet?.detail_species || '',
    birth_date: mockPet?.birth_date || '',
    gender: mockPet?.gender,
    about: mockPet?.about || '',
    profile_url: mockPet.profile_url || '',
  };
  ...
  const {
    ...
    resetInput: resetPetInput,
  } = useInput<IPet>(initialPetInput);

  const [editToggle, handleEditToggle] = useToggle(petInput === undefined);

  useEffect(() => {
    if (petInput.species !== mockPet?.species) setPetInput({ ...petInput, detail_species: '' });
  }, [petInput.species]);

  // petId를 prop으로 전달받아 pet 데이터 fetch후 PetDetailForm에 전달하기
  return (
    <section className="w-[54.125rem] mt-[5.25rem] flex items-start justify-between p-[2.5rem] bg-white">
      <ProfileImg editToggle={editToggle} input={petInput} setInput={setPetInput} />
      <PetDetailForm
        petInput={petInput}
